...
export default function AddClient(props){
  ...
  useEffect(() => {
    if(props.client){
      setname(props.client.username);
      setemail(props.client.email); 
      setpassword(props.client.password);
...
const ClickHandler = async () =>{  
  ...
  try {
   ...
   props.client ? props.onupdatehandler({...data, id: props.client.id}) :
    ...
    return(
         <div>
          <div style={{color:"red"}}>{error}</div>
                  ...
                  <input value={addpassword} onChange={(e)=>setpassword(e.target.value)} type="text" id="addpassword" placeholder='password'/>
                <button onClick={()=>ClickHandler()} style={{backgroundColor: "green", fontWeight: "bold"} }>{props.client ? "Update": "Add New"} User</button>
                </div>
    )
}